...
/**
 * @ClassName: WapServicePageService 
 * @Description: Wap端产品相关、价值账户查询相关以外的服务类页面
 * @author langxb
 * @date 2016年10月26日
 */
public class WapServicePageService implements Serializable {
    ...
    public DataFrame getUserBehaviorClueDF(HiveContext sqlContext, String appids) {
    	...
    	String paramSql = " AND trim(EVENT) <> '我的保单' AND trim(EVENT) <> '理财' AND trim(EVENT) <> '首页-精选' AND trim(EVENT) <> '账户价格查询页面首页' AND trim(EVENT) <> '净值公布'";
    	DataFrame teleDf = getDataFromStatisticsEventTable(sqlContext, appids, paramSql);
    	JavaRDD<FactUserBehaviorClue> labelRdd = analysisWapServicePageRDD(teleDf);
    ...
    public DataFrame fullFillUserNameByUserInfoTable(HiveContext sqlContext) {
    	String hql = "SELECT TMP_A.ROWKEY, "
                ...
                + "          TMP_A.VISIT_COUNT,"
                + "          TMP_A.CLUE_TYPE,"
                + "          CONCAT('姓名：',UM.NAME,'\073性别：',CASE WHEN UM.GENDER='0' THEN '男' WHEN UM.GENDER='1' THEN '女' END, '\073出生日期：',TO_DATE(UM.BIRTHDAY),'\073首次访问时间：',TMP_A.VISIT_TIME,'\073访问次数：',TMP_A.VISIT_COUNT,'\073访问时长：',TMP_A.VISIT_DURATION) AS REMARK "
	...
	public static JavaRDD<FactUserBehaviorClue> analysisWapServicePageRDD(DataFrame df) {
		JavaRDD<Row> jRDD = df.select("ROWKEY", "USER_ID", "APP_TYPE", "APP_ID", 
				"EVENT", "SUBTYPE", "LABEL", "CUSTOM_VAL", "VISIT_COUNT", "VISIT_TIME", "VISIT_DURATION","FROM_ID").rdd().toJavaRDD();
		JavaRDD<FactUserBehaviorClue> pRDD = jRDD.map(new Function<Row, FactUserBehaviorClue>() {
			...
			public FactUserBehaviorClue call(Row row) throws Exception {
				...
				String VISIT_DURATION = row.getString(10);
				String FROM_ID = row.getString(11);
				return SrcLogParse.analysisWapServicePageLable(ROWKEY, USER_ID, APP_TYPE,
						APP_ID, EVENT, SUBTYPE, LABEL, CUSTOM_VAL, VISIT_COUNT, VISIT_TIME, 
						VISIT_DURATION,FROM_ID);
			}
		});
